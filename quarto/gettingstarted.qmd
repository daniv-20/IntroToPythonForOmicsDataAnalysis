...
It is also possible to configure your IDE to play well with git/github and allow remote-ssh to a compute cluster (stay tuned!!).

# Step 3: Working with Python

Depending on your IDE, you may have to install an extension to work with Python. Other than that, if you added Python to the system path correctly, you sould be ready to go! 
...
You can also run python scripts from the command line. If you are using Positron, click on the "TERMINAL" tab **add img**. In the terminal, you should see the name of your working directory (the folder your 'first_python.py' document is in.) **show another image**

The 'scripting language' of the terminal is called 'bash' **make sure this is correct**. If you are working on MAC, you might have to do `python3 first_python.py` instead.
...
Finally, you can run one or multiple python files using a bash script (.sh file). This is a bit more involved so it will be included in another tutorial **link here when available**.
...
There are two ways to connect Positron to a remote host. The fast way is to click the little >< icon at the bottom left of the screen **add image**. Mine is pink, yours may be another color depending on your theme. This will take you directly to the options to `Connect to Host`, `Connect Current Window to Host` or `Update SSH Configuration` **add image**. Click `Connect Current Window to Host` and put the username you use to connect to your compute cluster **add img** and then hit enter and type your password. 
